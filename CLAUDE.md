...
| `internal/handler` | 内部 | HTTP handler；每个写路径 handler 注入 `pubsub.Hub`；auth handler 额外持有 `ratelimit.Limiter`、`store.Cache`、`billing.Provider`、`captcha.Verifier`、`mailer.Mailer` |
...
- **schema 位置**：schema 文件在 `db/schema.pg.sql`（PostgreSQL，`//go:embed` 引用），根目录的 `schema.sql` 仅作存档，不被代码引用。所有列已合并到 `CREATE TABLE` 定义，无迁移补丁。原有限流表（`login_failures`、`otp_ip_requests`、`register_ip_requests`、`sse_tokens`）不存在于 schema，对应状态由 `internal/ratelimit` 和 `internal/store` 管理（in-memory 或 Redis）。`groups` / `group_tabs` 两张表：`groups` 含 `seq`/`deleted_at`/`is_deleted`/`workspace_id` 同步字段；`group_tabs` 以 `group_id REFERENCES groups(id) ON DELETE CASCADE` 外键关联，tab 列表无 seq，整体快照替换。
