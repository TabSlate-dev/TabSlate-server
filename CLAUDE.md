...
```
cmd/server/main.go
  → local.New(licenseKey, database)             # 创建 OSS Provider（传 DB 供用户数查询）
  → bp.Start(ctx)                               # 机器激活 + 初始 License 同步 + 后台刷新 goroutine
...
- `KEYGEN_API_URL` 和 `KEYGEN_ACCOUNT_ID` 通过 `-ldflags -X` 编译时写入二进制，不允许运行时环境变量覆盖，防止重定向到伪造的 keygen.sh 端点
