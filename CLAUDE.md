...
```
cmd/server/main.go
  → local.New(licenseKey, pubkey, database)     # 创建 OSS Provider（传 DB 供容量表读写）
  → app.New(cfg, db, provider)                  # 注入（captcha + mailer 由 Config 自动创建）
  → handler.NewAuthHandler(db, secret, provider, captcha, mailer)
```

`app.New()` 内部根据 `Config` 的 Prosopo / Mail 字段自动创建 `captcha.Verifier` 和 `mailer.Mailer`，外部调用方无需导入 `internal/` 包。Cloud 仓库的 `main.go` 只需将 `local.New(...)` 替换为 `meteroid.New(...)`，其余不变。`RegisterWebhook()` 供 Cloud 额外挂载 webhook 路由。
...
| `billing/local` | 公开 | OSS 实现：从 `subscription_capacity` DB 表读取配额；启动时写入 `unlimited` 行（所有字段 -1）；DB 不可用时 fallback 到 -1 默认值 |
...
- **同步事务要求**：`SyncHandler.Push` 必须在单个 pgx 事务中完成配额检查 + 所有 upsert + `incrementSeq`，禁止将配额检查移到事务外（TOCTOU 竞争会导致超配额）。配额 SQL 条件：collections 用 `is_deleted = 0`（archived 计入，trashed 不计入），saved groups 用 `deleted_at IS NULL`；配额上限由 `billing.Provider.GetLimits()` 返回（事务外调用一次，结果在循环内复用）。groups 的 Push 包含两步：LWW upsert `groups` 行（`WHERE groups.updated_at < excluded.updated_at`）+ 原子替换 `group_tabs`（`DELETE FROM group_tabs WHERE group_id = $id` 后 bulk INSERT），两步均在同一事务内执行；stale group 被拒绝时跳过其 tab 替换。
