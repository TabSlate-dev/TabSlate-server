...
cmd/server/main.go
  ...
  → handler.NewAuthHandler(db, secret, provider, captcha, mailer, verifyBaseURL)
```
...
| `PROSOPO_SECRET` | | Prosopo 站点密钥；空 = 跳过验证码 |
| `PROSOPO_SERVER_URL` | | Prosopo 验证端点，默认 `https://api.prosopo.io/siteverify`；可指向自部署实例 |
...
| `VERIFY_BASE_URL` | | 邮箱验证链接的 base URL，如 `https://api.tabslate.app` |
...
- **schema 位置**：schema 文件在 `db/schema.pg.sql`（PostgreSQL，`//go:embed` 引用），根目录的 `schema.sql` 仅作存档，不被代码引用。schema 末尾包含 `DO $$ ... ALTER TABLE ... EXCEPTION WHEN duplicate_column` 块，确保对已有数据库的兼容迁移。
