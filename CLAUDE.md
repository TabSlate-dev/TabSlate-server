...
cmd/server/main.go
  ...
  → server.New(cfg, db, provider)  # 注入
  → handler.NewAuthHandler(db, secret, provider)
```

Cloud 仓库的 `main.go` 只需将 `local.New(...)` 替换为 `lago.New(...)`，其余不变。`RegisterWebhook()` 供 Cloud 额外挂载 Lago webhook 路由。
...
| `internal/handler` | 内部 | HTTP handler，每个 handler 持有 `*db.DB`，auth handler 额外持有 `billing.Provider` |
| `internal/plan` | 内部 | 基于本地 DB 的配额检查（过渡期保留，长期应迁移到 `billing.GetLimits`） |
| `internal/auth` | 内部 | JWT 签发/验证、bcrypt、refresh token 生成 |
| `internal/middleware` | 内部 | Bearer JWT 验证中间件，将 `userID` 注入 Gin context |
...
- `POST /auth/register` 成功后异步调 `billing.OnUserCreated`（goroutine）
...
- **schema 位置**：schema 文件在 `internal/db/schema.sql`（libsql）和 `internal/db/schema.pg.sql`（PostgreSQL），根目录的 `schema.sql` 仅作存档，不被代码引用
