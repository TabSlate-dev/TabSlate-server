...
`db.DB` 包装 `*pgxpool.Pool`，仅支持 PostgreSQL 17+（pgx/v5 驱动）。DSN 必须以 `postgres://` 或 `postgresql://` 开头，否则启动报错。

所有 handler 的 SQL 查询直接使用 PostgreSQL 原生占位符 `$1, $2...`，pgxpool 不使用 `?` 也没有 `Rebind` 方法。
...
| `db` | 公开 | DB 包装器（`*pgxpool.Pool`）、schema embed；Cloud 模块可导入 |
...
```go
// ✅ pgxpool：ctx 作为第一个参数，直接使用 $N 占位符
h.db.QueryRow(c.Request.Context(), `SELECT * FROM users WHERE id = $1`, id)
h.db.Exec(c.Request.Context(), `INSERT INTO ... VALUES ($1, $2)`, v1, v2)

// ❌ 不传 ctx：无法被取消，长查询会造成 goroutine 泄漏（pgxpool 不支持无 ctx 调用）
```
...
- 所有带参数的查询使用 PostgreSQL 原生占位符 `$1, $2...`，直接传给 pgxpool（无需 Rebind）
